...
Tests the full data pipeline from raw data ingestion through
...
class TestEndToEndPipeline:
    ...
    def test_complete_pipeline_execution(self, spark, test_data_paths,
        ...
        enriched_df = optimized_join(
            interactions_df, metadata_df, "user_id", hot_keys
        )
    ...
    def test_pipeline_with_large_dataset(self, spark, test_data_paths):
        ...
        from src.transforms.join_transforms import identify_hot_keys, optimized_join
        from src.transforms.engagement_transforms import calculate_dau, calculate_mau
        ...
        enriched_df = optimized_join(interactions_df, metadata_df, "user_id", hot_keys)
    ...
    def test_pipeline_data_lineage(self, spark, test_data_paths,
        ...
        enriched_df = optimized_join(
            sample_interactions_data, sample_metadata_data, "user_id", hot_keys
        )
    ...
    def test_pipeline_performance_benchmark(self, spark, sample_interactions_data,
                                           sample_metadata_data):
        """Benchmark complete pipeline performance."""
        import time
        from src.transforms.join_transforms import identify_hot_keys, optimized_join
        from src.transforms.engagement_transforms import calculate_dau, calculate_mau
        from src.transforms.performance_transforms import calculate_percentiles
        ...
        hot_keys = identify_hot_keys(sample_interactions_data, "user_id")
        enriched = optimized_join(sample_interactions_data, sample_metadata_data,
                                 "user_id", hot_keys)
        ...
        percentiles = calculate_percentiles(enriched, "duration_ms", ["app_version"],
                                           [0.5, 0.95, 0.99])
        percentiles.count()
    ...
    def test_pipeline_idempotency(self, spark, test_data_paths,
                                 sample_interactions_data, sample_metadata_data):
        ...
        assert results_1["dau_count"] == results_2["dau_count"], \
            "DAU counts should match"
        assert results_1["total_dau"] == results_2["total_dau"], \
            "Total DAU should match"

    def test_pipeline_with_monitoring_comprehensive(self, spark, sample_interactions_data,
                                                    sample_metadata_data):
